...
Method used internally in the `verifyCSProjVersionIncremented`, `verifyPackageJsonVersionIncremented` and `verifyFileVersionIncremented` method.

Takes two parameters of the versions to compare, typically master version and branch version.

The method returns `true` if both versions are valid SemVers, and the second version is higher than the first.

The method returns `false` if either version is invalid, or the second version is not higher than the first.
