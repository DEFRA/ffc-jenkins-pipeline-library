...
class Release implements Serializable {
  ...
  static def trigger(ctx, versionTag, repoName, releaseDescription, token){
    ...
    String releaseName = "Release ${versionTag}"
    ...
    def script = "curl -v -X POST -H 'Authorization: token $token' --data-urlencode 'tag_name=$versionTag' --data-urlencode 'name=$releaseName' --data-urlencode 'body=$releaseDescription' https://api.github.com/repos/DEFRA/$repoName/releases"
