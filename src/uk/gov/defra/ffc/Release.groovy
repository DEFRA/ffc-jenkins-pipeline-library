 ...
 */
  ...
  static def trigger(ctx, versionTag, repoName, releaseDescription, token){
    ...
    result = ctx.sh(returnStdout: true, script: "curl -s -X POST -H 'Authorization: token $token' -d '{ \"tag_name\" : \"$versionTag\", \"name\" : \"Release $versionTag\", \"body\" : \" test release description\" }' https://api.github.com/repos/DEFRA/$repoName/releases")
