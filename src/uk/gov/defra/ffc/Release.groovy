...
class Release implements Serializable {
  ...
  static def trigger(ctx, versionTag, repoName, releaseDescription, token){
    ...
    result = ctx.sh(returnStdout: true, script: "curl -v -X POST -H 'Authorization: token $token' --data-urlencode '{ \"tag_name\" : \"$versionTag\", \"name\" : \"Release $versionTag\", \"body\" : \" $releaseDescription \" }' https://api.github.com/repos/DEFRA/$repoName/releases")    
