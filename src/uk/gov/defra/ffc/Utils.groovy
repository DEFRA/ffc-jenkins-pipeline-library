...
class Utils implements Serializable {
  ...
  static def getConfigValues(ctx, searchKeys, appConfigPrefix, appConfigLabel=defaultNullLabel, escapeChars = true, environment = 'snd') {
    ...
    searchKeys.each { key ->
      ...
      if (appConfigMap.containsKey(searchKey)) {
        ...
        if (key == 'ingress.endpoint') {
          appConfigMap[searchKey] = "${appConfigMap[searchKey]}-${environment}"
        }
