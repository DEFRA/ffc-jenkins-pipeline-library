    ...
    ctx.withCredentials([
      ...
      ctx.dir('test-output') {
        ...
        for (pact in pacts) {
          ...
          ctx.sh "curl -k -v -XPUT -H \"Content-Type: application/json\" --user ${ctx.pactUsername}:${ctx.pactPassword} -d@${pact.name} ${ctx.PACT_BROKER_URL}/pacts/provider/$provider/consumer/$repoName/version/$version+$commitSha/tag/main"
        }
      }
    }
  }
