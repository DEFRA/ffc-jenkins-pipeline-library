    ...
    ctx.withCredentials([
      ...
      ctx.dir('test-output') {
        ...
        for (pact in pacts) {
          ...
          def script = "docker run --rm -w \$(pwd) -v \$(pwd):\$(pwd) -v /etc/ssl/certs/:/etc/ssl/certs/ -v /usr/local/share/ca-certificates/:/usr/local/share/ca-certificates/ -e SSL_CERT_DIR /etc/ssl/certs -e PACT_DISABLE_SSL_VERIFICATION=false -e PACT_BROKER_BASE_URL=$ctx.PACT_BROKER_URL -e PACT_BROKER_USERNAME=$ctx.pactUsername -e PACT_BROKER_PASSWORD=$ctx.pactPassword pactfoundation/pact-cli:latest broker publish --consumer-app-version $version+$commitSha $pact --tag main"
