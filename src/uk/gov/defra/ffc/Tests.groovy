...
class Tests implements Serializable {
  ...
  static def runAccessibility(ctx, projectName, buildNumber, tag, accessibilityTestType) {
    def dockerComposeFile = 'docker-compose.$accessibilityTestType.yaml'
      ctx.gitStatusWrapper(credentialsId: 'github-token', sha: Utils.getCommitSha(ctx), repo: Utils.getRepoName(ctx), gitHubContext: GitHubStatus[accessibilityTestType.capitalize()]['Context'], description: GitHubStatus[accessibilityTestType.capitalize()]['Description']) {
        try {
          ctx.sh('mkdir -p -m 666 test-output')
