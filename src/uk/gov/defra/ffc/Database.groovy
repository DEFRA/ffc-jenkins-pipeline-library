...
class Database implements Serializable {
  ...
  static def provisionPrDbRoleAndSchema(ctx, host, dbName, jenkinsUserCredId, prUserCredId, prCode, useIfNotExists=false) {
    ctx.withCredentials([
      ...
      if (useIfNotExists) {
        ...
        roleExists = runPsqlCommand(ctx, ctx.dbHost, ctx.dbUser, dbName, selectRoleSqlCmd).contains('(1 row)')
      }
      ...
      if (roleExists) {
        ...
        runPsqlCommand(ctx, ctx.dbHost, ctx.dbUser, dbName, createRoleSqlCmd)
      }

      def ifNotExistsStr = useIfNotExists ? 'IF NOT EXISTS' : ''
      def createSchemaSqlCmd = "CREATE SCHEMA $ifNotExistsStr $prSchema"
      runPsqlCommand(ctx, ctx.dbHost, ctx.dbUser, dbName, createSchemaSqlCmd)

      def grantPrivilegesSqlCmd = "GRANT ALL PRIVILEGES ON SCHEMA $prSchema TO $prUser"
      runPsqlCommand(ctx, ctx.dbHost, ctx.dbUser, dbName, grantPrivilegesSqlCmd)

      def setSearchPathCmd = "ALTER ROLE $prUser SET search_path TO $prSchema"
      runPsqlCommand(ctx, ctx.dbHost, ctx.dbUser, dbName, setSearchPathCmd)
  ...
  static def destroyPrDbRoleAndSchema(ctx, host, dbName, jenkinsUserCredId, prCode) {
    ctx.withCredentials([
      ...
      runPsqlCommand(ctx, ctx.dbHost, ctx.dbUser, dbName, dropSchemaSqlCmd)

      def dropRoleSqlCmd = "DROP ROLE IF EXISTS $prUser"
      runPsqlCommand(ctx, ctx.dbHost, ctx.dbUser, dbName, dropRoleSqlCmd)
    }
  }
