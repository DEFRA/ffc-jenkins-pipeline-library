...
class Build implements Serializable {
  ...
  static def checkoutSourceCode(ctx, defaultBranch) {
    ctx.withCredentials([
      ctx.usernamePassword(credentialsId: 'github-token', usernameVariable: 'username', passwordVariable: 'password')
    ]) {
  ...
  static def getVariables(ctx, version, defaultBranch) {
    ctx.withCredentials([
      ...
      def pr = ctx.sh(returnStdout: true, script: "curl -H 'Authorization: token $ctx.gitToken' https://api.github.com/repos/DEFRA/$repoName/pulls?state=open | jq '.[] | select(.head.ref == \"$branch\") | .number'").trim()
  ...
  static def snykTest(ctx, failOnIssues, organisation, severity, targetFile, pr) {
    ...
    ctx.dir('snyk-cli') {
      ctx.withCredentials([ctx.string(credentialsId: 'ffc-snyk-token', variable: 'snykToken')
          ...
          def script = "docker run -e 'SNYK_TOKEN=$ctx.snykToken' -e 'MONITOR=true' -v '$ctx.WORKSPACE:/project' snyk/snyk-cli:npm test --json > snyk-result.json --org=$organisation --severity-threshold=$severity --file=$targetFile --strict-out-of-sync=false"
