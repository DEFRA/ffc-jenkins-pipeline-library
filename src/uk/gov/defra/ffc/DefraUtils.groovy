...
def releaseExists(containerTag, repoName, token){
    ...
    result = sh(returnStdout: true, script: "curl --silent -H 'Authorization: token $token' https://api.github.com/repos/DEFRA/$repoName/releases | jq '.[].tag_name | index(\"$containerTag\") | select (. != null) | tostring'").trim()
    echo $result
    if (result == "0"){
      echo "Release exists!"
