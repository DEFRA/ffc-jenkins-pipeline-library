...
def releaseExists(containerTag, repoName, token){
    ...
    def result = sh(returnStdout: true, script: "curl -s -H 'Authorization: token $token' https://api.github.com/repos/DEFRA/$repoName/releases/tags/$containerTag | jq '.tag_name'.execute.text") 
    echo "The result is $result"
    if (result.equals(containerTag)){
      echo "Release exists!"
